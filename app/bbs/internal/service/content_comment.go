...
func NewContentCommentService(
	...
	return &ContentCommentService{
		contentCommentUsecase: contentCommentUsecase,
	}
}
...
func (s *ContentCommentService) Create(ctx context.Context, req *bbscontentv1.CreateComment_Req) (*bbscontentv1.CreateComment_Resp, error) {
	...
	if resp != nil {
		comment = &bbscontentv1.CreateComment_Resp_CommentDetail{
			Id:            resp.ID,
		...
		if resp.ViewerActionState != nil {
			comment.ViewerActionState = &bbscontentv1.CreateComment_Resp_CommentViewerActionState{
				Liked:   resp.ViewerActionState.Liked,
				Thanked: resp.ViewerActionState.Thanked,
			}
		}
		if resp.User != nil {
			comment.User = &bbscontentv1.CreateComment_Resp_AccountProfile{
				Id:            resp.User.ID,
				Name:          resp.User.Name,
				Nickname:      resp.User.Nickname,
				Url:           resp.User.URL,
				AvatarUrl:     resp.User.AvatarURL,
				Introduction:  resp.User.Introduction,
				Status:        bbsuserv1enum.AccountStatus(resp.User.Status),
		...
		if resp.ReplyUser != nil {
			comment.ReplyUser = &bbscontentv1.CreateComment_Resp_AccountProfile{
				Id:            resp.ReplyUser.ID,
				Name:          resp.ReplyUser.Name,
				Nickname:      resp.ReplyUser.Nickname,
				Url:           resp.ReplyUser.URL,
				AvatarUrl:     resp.ReplyUser.AvatarURL,
				Introduction:  resp.ReplyUser.Introduction,
				Status:        bbsuserv1enum.AccountStatus(resp.ReplyUser.Status),
	...
	return &bbscontentv1.CreateComment_Resp{
		Comment: comment,
	}, nil
}
...
func (s *ContentCommentService) List(ctx context.Context, req *bbscontentv1.ListComments_Req) (*bbscontentv1.ListComments_Resp, error) {
	...
	for _, row := range resp.Rows {
		...
		item := &bbscontentv1.ListComments_Resp_CommentListItem{
			Id:            row.ID,
		...
		if row.ViewerActionState != nil {
			item.ViewerActionState = &bbscontentv1.ListComments_Resp_CommentViewerActionState{
				Liked:   row.ViewerActionState.Liked,
				Thanked: row.ViewerActionState.Thanked,
			}
		}
		if row.User != nil {
			item.User = &bbscontentv1.ListComments_Resp_AccountProfile{
				Id:            row.User.ID,
				Name:          row.User.Name,
				Nickname:      row.User.Nickname,
				Url:           row.User.URL,
				AvatarUrl:     row.User.AvatarURL,
				Introduction:  row.User.Introduction,
				Status:        bbsuserv1enum.AccountStatus(row.User.Status),
		...
		if row.ReplyUser != nil {
			item.ReplyUser = &bbscontentv1.ListComments_Resp_AccountProfile{
				Id:            row.ReplyUser.ID,
				Name:          row.ReplyUser.Name,
				Nickname:      row.ReplyUser.Nickname,
				Url:           row.ReplyUser.URL,
				AvatarUrl:     row.ReplyUser.AvatarURL,
				Introduction:  row.ReplyUser.Introduction,
				Status:        bbsuserv1enum.AccountStatus(row.ReplyUser.Status),
...
func (s *ContentCommentService) ListThreads(ctx context.Context, req *bbscontentv1.ListCommentThreads_Req) (*bbscontentv1.ListCommentThreads_Resp, error) {
	...
	for _, row := range resp.Rows {
		...
		thread := &bbscontentv1.ListCommentThreads_Resp_CommentThread{
			ReplyCount:     row.ReplyCount,
			HasMoreReplies: row.HasMoreReplies,
		}
		if row.Root != nil {
			thread.Root = &bbscontentv1.ListCommentThreads_Resp_CommentListItem{
				Id:            row.Root.ID,
				ArticleId:     row.Root.ArticleID,
				Content:       row.Root.Content,
				ContentRender: row.Root.ContentRender,
				Level:         row.Root.Level,
				ParentId:      row.Root.ParentID,
				ReplyId:       row.Root.ReplyID,
				ReplyCount:    row.Root.ReplyCount,
			...
			if row.Root.ViewerActionState != nil {
				thread.Root.ViewerActionState = &bbscontentv1.ListCommentThreads_Resp_CommentViewerActionState{
					Liked:   row.Root.ViewerActionState.Liked,
					Thanked: row.Root.ViewerActionState.Thanked,
				}
			}
			if row.Root.User != nil {
				thread.Root.User = &bbscontentv1.ListCommentThreads_Resp_AccountProfile{
					Id:            row.Root.User.ID,
					Name:          row.Root.User.Name,
					Nickname:      row.Root.User.Nickname,
					Url:           row.Root.User.URL,
					AvatarUrl:     row.Root.User.AvatarURL,
					Introduction:  row.Root.User.Introduction,
					Status:        bbsuserv1enum.AccountStatus(row.Root.User.Status),
			...
			if row.Root.ReplyUser != nil {
				thread.Root.ReplyUser = &bbscontentv1.ListCommentThreads_Resp_AccountProfile{
					Id:            row.Root.ReplyUser.ID,
					Name:          row.Root.ReplyUser.Name,
					Nickname:      row.Root.ReplyUser.Nickname,
					Url:           row.Root.ReplyUser.URL,
					AvatarUrl:     row.Root.ReplyUser.AvatarURL,
					Introduction:  row.Root.ReplyUser.Introduction,
					Status:        bbsuserv1enum.AccountStatus(row.Root.ReplyUser.Status),
		...
		for _, preview := range row.PreviewReplies {
			...
			item := &bbscontentv1.ListCommentThreads_Resp_CommentListItem{
				Id:            preview.ID,
				ArticleId:     preview.ArticleID,
				Content:       preview.Content,
				ContentRender: preview.ContentRender,
				Level:         preview.Level,
			...
			if preview.ViewerActionState != nil {
				item.ViewerActionState = &bbscontentv1.ListCommentThreads_Resp_CommentViewerActionState{
					Liked:   preview.ViewerActionState.Liked,
					Thanked: preview.ViewerActionState.Thanked,
				}
			}
			if preview.User != nil {
				item.User = &bbscontentv1.ListCommentThreads_Resp_AccountProfile{
					Id:            preview.User.ID,
					Name:          preview.User.Name,
					Nickname:      preview.User.Nickname,
					Url:           preview.User.URL,
					AvatarUrl:     preview.User.AvatarURL,
					Introduction:  preview.User.Introduction,
					Status:        bbsuserv1enum.AccountStatus(preview.User.Status),
			...
			if preview.ReplyUser != nil {
				item.ReplyUser = &bbscontentv1.ListCommentThreads_Resp_AccountProfile{
					Id:            preview.ReplyUser.ID,
					Name:          preview.ReplyUser.Name,
					Nickname:      preview.ReplyUser.Nickname,
					Url:           preview.ReplyUser.URL,
					AvatarUrl:     preview.ReplyUser.AvatarURL,
					Introduction:  preview.ReplyUser.Introduction,
					Status:        bbsuserv1enum.AccountStatus(preview.ReplyUser.Status),
	...
	return &bbscontentv1.ListCommentThreads_Resp{
		Page: page,
		Rows: rows,
	}, nil
}
...
func (s *ContentCommentService) ListReplies(ctx context.Context, req *bbscontentv1.ListCommentReplies_Req) (*bbscontentv1.ListCommentReplies_Resp, error) {
	...
	for _, row := range resp.Rows {
		...
		item := &bbscontentv1.ListCommentReplies_Resp_CommentListItem{
			Id:            row.ID,
			ArticleId:     row.ArticleID,
			Content:       row.Content,
			ContentRender: row.ContentRender,
		...
		if row.ReplyUser != nil {
			item.ReplyUser = &bbscontentv1.ListCommentReplies_Resp_AccountProfile{
				Id:            row.ReplyUser.ID,
				Name:          row.ReplyUser.Name,
				Nickname:      row.ReplyUser.Nickname,
				Url:           row.ReplyUser.URL,
	...
	return &bbscontentv1.ListCommentReplies_Resp{
		Page: page,
		Rows: rows,
	}, nil
}
...
func (s *ContentCommentService) ListTimeline(ctx context.Context, req *bbscontentv1.ListCommentTimeline_Req) (*bbscontentv1.ListCommentTimeline_Resp, error) {
	...
	for _, row := range resp.Rows {
		...
		item := &bbscontentv1.ListCommentTimeline_Resp_CommentListItem{
			Id:            row.ID,
			ArticleId:     row.ArticleID,
			Content:       row.Content,
			ContentRender: row.ContentRender,
		...
		if row.ReplyUser != nil {
			item.ReplyUser = &bbscontentv1.ListCommentTimeline_Resp_AccountProfile{
				Id:            row.ReplyUser.ID,
				Name:          row.ReplyUser.Name,
				Nickname:      row.ReplyUser.Nickname,
				Url:           row.ReplyUser.URL,
	...
	return &bbscontentv1.ListCommentTimeline_Resp{
		Page: page,
		Rows: rows,
	}, nil
}
...
func (s *ContentCommentService) Like(ctx context.Context, req *bbscontentv1.LikeComment_Req) (*bbscontentv1.LikeComment_Resp, error) {
	...
	return &bbscontentv1.LikeComment_Resp{
		Liked: resp,
	}, nil
}
...
func (s *ContentCommentService) Thank(ctx context.Context, req *bbscontentv1.ThankComment_Req) (*bbscontentv1.ThankComment_Resp, error) {
	...
	return &bbscontentv1.ThankComment_Resp{
		Thanked: resp,
	}, nil
}
