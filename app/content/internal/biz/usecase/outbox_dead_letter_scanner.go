	...
	go func() {
		for {
			if err := s.scan(runCtx); err != nil {
				s.log.Error(fmt.Sprintf("scan outbox dead letters failed: %v", err))
			}
			select {
			case <-runCtx.Done():
				return
...
func (s *OutboxDeadLetterScanner) scan(ctx context.Context) error {
	...
	pageResp, err := s.outboxRepo.Page(ctx, &repo.OutboxEventGetReq{Page: &base.PageRequest{Page: 1, Size: outboxDeadLetterScanLimit}, Status: &status})
	if err != nil {
		return err
	}
	for _, row := range pageResp.Rows {
		...
		if err := s.alertClient.Alert(ctx, s.deadLetterConf(), s.alertConf(), &commonClient.DeadLetterAlert{
			...
			s.log.Warn(fmt.Sprintf("alert outbox dead letter failed: event_id=%s err=%v", row.EventID, err))
		}
	}
	return nil
}
