...
func (Article) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("restriction").Values(contentenum.ContentRestrictionMap.EnumValues()...).Default(contentenum.ContentRestrictionNone.String()).Comment("治理限制"),
		field.Enum("type").Values(contentenum.ArticleTypeMap.EnumValues()...).Default(contentenum.ArticleTypeNormal.String()).Comment("文章类型"),
		...
		field.Int32("reward_count").Comment("打赏数").Default(0),
...
func (Article) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.To("view_records", ArticleViewRecord.Type),
	}
}
