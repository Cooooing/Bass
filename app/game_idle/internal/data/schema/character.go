...
func (Character) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("nickname").Comment("角色昵称").MaxRuneLen(32).NotEmpty(),
		field.String("nickname_key").Comment("小写角色昵称，用于大小写不敏感唯一约束").MaxLen(32).NotEmpty(),
...
func (Character) Indexes() []ent.Index {
	return []ent.Index{
		...
		index.Fields("nickname_key").
			Unique().
			StorageKey("game_idle_characters_nickname_key_active_unique").
			Annotations(entsql.IndexWhere("deleted_at IS NULL")),
