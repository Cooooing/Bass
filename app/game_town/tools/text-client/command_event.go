...
func listEvents(ctx context.Context, client *rpc.GameTownClient, playerID int64, worldID int64) commandResult {
	...
	reply, err := client.Event.Page(ctx, &v1.PageGameTownEvents_Request{Page: &common.PageReq{Page: 1, Size: 20}, WorldId: worldID, PlayerId: playerID})
	if err != nil {
		return commandResult{err: err}
	}
	if len(reply.GetRows()) == 0 {
		return commandResult{lines: []string{"暂无世界事件"}}
	}
	return commandResult{lines: lo.Map(reply.GetRows(), func(row *v1.PageGameTownEvents_Resp_Row, _ int) string {
