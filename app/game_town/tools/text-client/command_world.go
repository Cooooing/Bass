...
func useWorld(ctx context.Context, client *rpc.GameTownClient, value string) commandResult {
	...
	reply, err := client.World.Get(ctx, &v1.GetGameTownWorld_Request{Id: worldID})
	if err != nil {
...
func listWorlds(ctx context.Context, client *rpc.GameTownClient) commandResult {
	reply, err := client.World.Page(ctx, &v1.PageGameTownWorlds_Request{Page: &common.PageReq{Page: 1, Size: 100}})
	if err != nil {
		return commandResult{err: err}
	}
	if len(reply.GetRows()) == 0 {
		return commandResult{lines: []string{"暂无世界"}}
	}
	return commandResult{lines: lo.Map(reply.GetRows(), func(row *v1.PageGameTownWorlds_Resp_Row, _ int) string {
...
func createWorld(ctx context.Context, client *rpc.GameTownClient, playerID int64, parts []string) commandResult {
	if playerID == 0 {
		return commandResult{err: fmt.Errorf("请先使用 /register <name> 注册玩家，或 /player use <player_id>")}
	}
	configID, err := strconv.ParseInt(parts[2], 10, 64)
	if err != nil || configID <= 0 {
		return commandResult{err: fmt.Errorf("invalid config id")}
	}
...
func joinWorld(ctx context.Context, client *rpc.GameTownClient, playerID int64, parts []string) commandResult {
	...
	if strings.HasPrefix(worldCode, "<") && strings.HasSuffix(worldCode, ">") {
		return commandResult{err: fmt.Errorf("请把 %s 替换成 /world list 中显示的真实世界 code，例如 /world join wdkxxxx 我的角色倾向", worldCode)}
	}
