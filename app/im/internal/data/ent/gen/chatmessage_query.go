...
func (_q *ChatMessageQuery) QueryGroup() *ChatGroupQuery {
...
// QueryLastMessageGroup chains the current query on the "last_message_group" edge.
func (_q *ChatMessageQuery) QueryLastMessageGroup() *ChatGroupQuery {
	query := (&ChatGroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryLastMessageSession chains the current query on the "last_message_session" edge.
func (_q *ChatMessageQuery) QueryLastMessageSession() *ChatSessionQuery {
...
func (_q *ChatMessageQuery) WithGroup(opts ...func(*ChatGroupQuery)) *ChatMessageQuery {
	query := (&ChatGroupClient{config: _q.config}).Query()
...
// WithLastMessageGroup tells the query-builder to eager-load the nodes that are connected to
// the "last_message_group" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ChatMessageQuery) WithLastMessageGroup(opts ...func(*ChatGroupQuery)) *ChatMessageQuery {
	query := (&ChatGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithLastMessageSession tells the query-builder to eager-load the nodes that are connected to
// the "last_message_session" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ChatMessageQuery) WithLastMessageSession(opts ...func(*ChatSessionQuery)) *ChatMessageQuery {
	query := (&ChatSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withLastMessageSession = query
	return _q
}
...
//		GroupBy(chatmessage.FieldSenderID).
//		Aggregate(gen.Count()).
//		Scan(ctx, &v)
...
//
//	client.ChatMessage.Query().
//		Select(chatmessage.FieldSenderID).
//		Scan(ctx, &v)
...
func (_q *ChatMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChatMessage, error) {
	...
	if query := _q.withGroup; query != nil {
		if err := _q.loadGroup(ctx, query, nodes, nil,
			func(n *ChatMessage, e *ChatGroup) { n.Edges.Group = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withLastMessageSession; query != nil {
		if err := _q.loadLastMessageSession(ctx, query, nodes, nil,
			func(n *ChatMessage, e *ChatSession) { n.Edges.LastMessageSession = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChatMessageQuery) loadGroup(ctx context.Context, query *ChatGroupQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *ChatGroup)) error {
...
func (_q *ChatMessageQuery) loadLastMessageGroup(ctx context.Context, query *ChatGroupQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *ChatGroup)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chatgroup.FieldLastMessageID)
	}
	query.Where(predicate.ChatGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chatmessage.LastMessageGroupColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ChatMessageQuery) loadLastMessageSession(ctx context.Context, query *ChatSessionQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *ChatSession)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chatsession.FieldLastMessageID)
	}
	query.Where(predicate.ChatSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chatmessage.LastMessageSessionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
