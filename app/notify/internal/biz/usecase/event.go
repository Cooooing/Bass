...
func (u *EventUsecase) HandleMessage(ctx context.Context, req *EventHandleMessageReq) error {
	...
	if !claimed {
		claimedRetry, err := u.inboxEventRepo.ClaimRetry(ctx, &repo.InboxEventClaimRetryReq{
			EventID:           event.EventId,
			Now:               now,
			ProcessingTimeout: u.inboxProcessingTimeout(),
			MaxRetry:          u.inboxMaxRetry(),
		})
		if err != nil {
			return err
		}
	...
	if err != nil {
		return u.markFailed(ctx, &markFailedReq{
			EventID: event.EventId,
			Err:     err,
		})
	}
	if len(rules) == 0 {
		err = u.inboxEventRepo.MarkProcessed(ctx, &repo.InboxEventMarkProcessedReq{
			EventID: event.EventId,
			Now:     now,
		})
		return err
	...
	if err = u.notifyUsecase.Process(ctx, &NotifyProcessReq{
