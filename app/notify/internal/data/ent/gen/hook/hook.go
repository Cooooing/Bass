...
// The NotificationMetaFunc type is an adapter to allow the use of ordinary
// function as NotificationMeta mutator.
type NotificationMetaFunc func(context.Context, *gen.NotificationMetaMutation) (gen.Value, error)

// Mutate calls f(ctx, m).
func (f NotificationMetaFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	if mv, ok := m.(*gen.NotificationMetaMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *gen.NotificationMetaMutation", m)
}
