...
func (DelayedTask) Indexes() []ent.Index {
	return []ent.Index{
		index.Fields("task_key").Unique().StorageKey("scheduler_delayed_tasks_task_key_active_unique").Annotations(entsql.IndexWhere("deleted_at IS NULL")),
		index.Fields("handler_name").StorageKey("scheduler_delayed_tasks_handler_name_idx").Annotations(entsql.IndexWhere("deleted_at IS NULL")),
