...
import (
	...
	stream2 "common/pkg/cutil/stream"
	"context"
	"reflect"
)
...
func (m *ComparableMap[K, V]) Map() map[K]V {
	return *m
}
...
func (m *ComparableMap[K, V]) Stream(ctx context.Context) stream2.Stream[*Entry[K, V]] {
	return stream2.OfBlock[*Entry[K, V]](ctx, m.Entries()...)
}
